...
class ImageContentWidget extends StatelessWidget {
  ...
  ImageContentWidget(this.smallFotoItems, this.itemTypeEnum, this.authModeEnum, this.keywordService, {Key key, this.onPop}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Wrap(
      alignment: WrapAlignment.spaceBetween,
      children: smallFotoItems.map((item) => item.itemType == itemTypeEnum?SmallFotoItemWidget(item, authModeEnum, keywordService, onPop: onPop):null).toList().where((k) => k != null).toList(),
    );
  }
}